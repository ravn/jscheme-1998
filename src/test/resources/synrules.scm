...
(define (make-sid name expression control) (list name expression control #f))
(define sid-name first)
(define sid-expression second)
(define sid-control third)
(define (sid-output-expression x) (third (rest x)))
(define (set-sid-output-expression! x) (set-car! (list-tail 3 x)))
(define (make-ellipsis sids) (list sids))
(define ellipsis-sids first)
(define set-ellipsis-sids! set-car!)
